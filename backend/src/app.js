...
let webhookSetupStatus = config.telegram.enabled ? 'pending' : 'disabled';

async function ensureProductionTelegramWebhook() {
  if (
  ...
  webhookSetupPromise = Promise.all([
    telegram.registerWebhook(),
    telegram.registerCommands(),
  ])
    .then(() => {
      webhookSetupStatus = 'ready';
    })
    .catch((err) => {
      webhookSetupStatus = 'error';
...
function authorizeAdminRequest(req, res) {
  if (!config.cronSecret) {
    if (!config.isVercel && process.env.NODE_ENV !== 'production') return true;
    res.status(503).json({ ok: false, error: 'CRON_SECRET is not configured' });
    return false;
  }

  if (req.get('authorization') !== `Bearer ${config.cronSecret}`) {
...
app.get('/', async (_req, res) => {
  await ensureProductionTelegramWebhook();
  res.json({
    service: 'weather-daily-backend',
    status:
      config.isVercel && storageBackend() !== 'upstash-redis'
        ? 'degraded'
        : 'ok',
    runtime: config.isVercel ? 'vercel' : 'server',
    storage: storageBackend(),
    channels: enabledChannels(),
    telegramWebhook: webhookSetupStatus,
...
// Vercel Cron invokes this endpoint once per day with CRON_SECRET.
...
async function setLocationByText(chatId, raw) {
  ...
  try {
    ...
    await setAwaitingLocation(chatId, false);
  ...
  try {
    ...
    if (setCmd) {
      if (!(await setLocationByText(chatId, setCmd[1].trim()))) {
        await setAwaitingLocation(chatId, true);
    ...
    if (await isAwaitingLocation(chatId)) {
